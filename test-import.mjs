...
async function testImport() {
  const loginRes = await fetch('http://localhost:5000/api/hospital/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'vigneshvignesh1974@gmail.com', password: 'password123' })
  });
  ...
  const res = await fetch('http://localhost:5000/api/hospital/donors/register', {
    method: 'POST',
    headers: { 
