...
async function seedFinalData() {
  console.log('🚀 Starting Final Data Seeding for Demo...');
  ...
  try {
    ...
    for (const h of hospitals) {
      console.log(`🏥 Upserting hospital: ${h.name}`);
      await client.query(`
    ...
    for (const h of hospitals) {
      console.log(`👤 Seeding 75 records for ${h.name}...`);
      ...
      for (let i = 1; i <= 75; i++) {
        ...
        const name = `${h.city} Patient ${i}`;
        const donorName = `${h.city} Donor ${i}`;
        ...
        `, [h.id, 1000 + i, name, age, gender, bloodType, ogid_p, organ, urgencyLevels[Math.floor(Math.random() * 4)]]);
