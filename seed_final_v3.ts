...
async function seedFinalData() {
  console.log('🚀 Starting Final Data Seeding for Demo (V3)...');
  ...
  try {
    ...
    // Quick Count Check
    const pCount = await client.query('SELECT COUNT(*) FROM patients WHERE hospital_id IN ($1, $2)', [hospitals[0].id, hospitals[1].id]);
    const dCount = await client.query('SELECT COUNT(*) FROM donors WHERE hospital_id IN ($1, $2)', [hospitals[0].id, hospitals[1].id]);
    console.log(`📊 Final Counts for new hospitals -> Patients: ${pCount.rows[0].count}, Donors: ${dCount.rows[0].count}`);
